...
GreedPage::GreedPage(QWidget* parent) :
	PageWidget(parent)
{
#if DEBUG
	Logger::debug("Begin initialized GreedPage");
#endif // DEBUG
	...
	_mainOperateLayout->addWidget(greedCanvas); //, 4
...
#if DEBUG
	Logger::debug("------- End initialized GreedPage -------");
#endif // DEBUG
}

GreedPage::~GreedPage() {}

void GreedPage::autoResizeSettingsPage() {
	greedCanvas->settingPage()->autoResize();
}

PushButton* GreedPage::getPageIconButton(QWidget* context) {
...
PushButton* GreedPage::getPageTextButton(QWidget* context) {
	// Check for existed button
