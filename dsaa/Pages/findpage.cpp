...
{
#if DEBUG
	Logger::debug("Begin initialized find");
#endif // DEBUG
	...
	_mainOperateLayout->addWidget(findCanvas); //, 4
	...
	_titleLabel = new QLabel("Find algorithm", _contentWidget);
	_titleLabel->setFont(_titleFont);
	SlidePage* page = findCanvas->settingPage();
	...
	_pageDesc->setText("Visualization of find algorithm");
...
void FindPage::autoResizeSettingsPage() {
	findCanvas->settingPage()->autoResize();
}

PushButton* FindPage::getPageIconButton(QWidget* context) {
	...
	_iconButtonLabel->setText(FontAwesomeIcons::Instance()->getIconChar(FontAwesomeIcons::IconIdentity::icon_search));
...
PushButton* FindPage::getPageTextButton(QWidget* context) {
	// Check for existed button
	...
	_textButtonIcon->setText(FontAwesomeIcons::Instance()->getIconChar(FontAwesomeIcons::IconIdentity::icon_search));
